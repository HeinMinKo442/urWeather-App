import { formatDateOnly } from "../utls/TImeManagement";
...
function ShowDailyForecast({ data }) {
  ...
  return (
    <div className="mb-10">
      {data && (
        <h2 className="text-xl font-bold mb-4 underline-offset-1 text-center">
          5-Day Forecast
        </h2>
      )}
      <div className="flex gap-5 flex-row flex-wrap justify-center items-center mt-5">
        {data &&
          data.map((item) => (
            <div key={item.dt} className="border-2 rounded-lg p-4 text-center">
